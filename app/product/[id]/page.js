...
export default function ProductDetail() {
    ...
    const prevImage = () => handleSelectImage(Math.max(0, selectedImage - 1))
    const nextImage = () => handleSelectImage(Math.min(images.length - 1, selectedImage + 1))

    const addProductToCart = async () => {
        if (product.stock_quantity === 0) {
            setCartAddedAlert(true)
            setTimeout(() => setCartAddedAlert(null), 3000)
            return
        }
                ...
                <div className='m-10 bg-white rounded-4xl shadow-2xl'>
                    <div className='flex flex-col sm:flex-row  gap-2 '>
                        ...
                        <div className='flex flex-col items-center p-6 gap-4 border-gray-100'>
                            <div className='relative sm:w-100 sm:h-100 w-60 h-60 md:w-80 md:h-80 bg-gray-100 rounded-2xl overflow-hidden group'>
                                {images.length > 0 ? (
                                    <>
                                        <div
                                            className='flex h-full transition-transform duration-300 ease-in-out'
                                            style={{ width: `${images.length * 100}%`, transform: `translateX(-${selectedImage * (100 / images.length)}%)` }}
                                        >
                                            {images.map((url, i) => (
                                                <div key={i} className='relative h-full shrink-0' style={{ width: `${100 / images.length}%` }}>
                                                    <Image src={url} alt={`${product.product_name} ${i + 1}`} fill className='object-contain' />
                                        ...
                                        {selectedImage > 0 && (
                                            <button
                                                onClick={prevImage}
                                                className='absolute left-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-white/80 hover:bg-white shadow flex items-center justify-center text-gray-700 opacity-0 group-hover:opacity-100 transition-opacity'
                                            >
                                                &#10094;
                                            </button>
                                        )}
                                        {selectedImage < images.length - 1 && (
                                            <button
                                                onClick={nextImage}
                                                className='absolute right-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-white/80 hover:bg-white shadow flex items-center justify-center text-gray-700 opacity-0 group-hover:opacity-100 transition-opacity'
                                            >
                                                &#10095;
                                            </button>
                                        )}
                        ...
                        <div className='gap-2'>
                            ...
                            <div className='p-6 gap-3'>
                                ...
                                <div className={`inline-flex items-center border rounded bg-white w-fit ${
                                    product.stock_quantity === 0 ? 'text-gray-200' : 'text-black'
                                }`}>
                                    <button disabled={product.stock_quantity === 0} onClick={() => setQty(q => Math.max(1, q - 1))} className='px-3 py-1'>-</button>
                                    <span className='px-4'>{qty}</span>
                                    <button disabled={product.stock_quantity === 0} onClick={() => setQty(q => q + 1)} className='px-3 py-1'>+</button>
                                </div>
                                </div>
                                <div className='mt-5 flex gap-2'>
                                    <button onClick={addProductToCart} className='bg-white w-full py-2 border-2 border-gray-300 hover:bg-gray-300'>
                                        Add to Cart
                                    </button>

                                    {cartAddedAlert && (
                                        <div className={`fixed bottom-5 right-5 px-4 py-2 rounded-xl shadow-lg 
                                            ${product.stock_quantity === 0
                                                ? 'bg-red-500 text-white'
                                                : 'bg-green-200 text-[#4f4f4f]'
                                        }`}>
                                        {product.stock_quantity === 0
                                            ? 'Unable to add to cart. (The product is out of stock.)'
                                            : 'Added to Your Cart'}
                                        </div>
                                    )}
                                    ...
                                    {buyAlert && (
                                        ...
                                        >
                                        {product.stock_quantity === 0 && 'Unable to buy. (The product is out of stock.)'}
                                        </div>
                                    )}
                                </div>
                            </div>
